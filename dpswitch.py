...
class DisplayModeButton:
    ...
    def setPrimaryDisplay(self, name: str):
        ...
        """
        command = ["xrandr", "--output", name, "--primary"]
    ...
    def callback(self):
        ...
        for line in info_output.decode("utf-8").split("\n"):
            ...
            if re.match(
                r"[0-9a-zA-Z\-]* (connected|connected primary) [0-9]*x[0-9]*\+[0-9]*\+[0-9]+",
                line,
            ):
...
def main(configPath: str):
    ...
    if configJson is not None:
        for config_name in configJson["configs"]:
            configs = (
                configJson["configs"][config_name]
                if isinstance(configJson["configs"][config_name], list)
                else [configJson["configs"][config_name]]
            )
            for d in configs:
    ...
    if configJson is not None:
        ...
        for mode in configJson["configs"]:
            buttons.append(
                DisplayModeButton(mode, configJson["configs"][mode], statusbar)
            )
